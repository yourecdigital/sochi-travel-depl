...
# Ubuntu Server Setup Script for Sochi Travel Deployment
...
show_completion_info() {
    ...
    echo "2. Copy .env.example to .env and configure your secrets:"
    echo "   cp $PROJECT_DIR/.env.example $PROJECT_DIR/.env"
    echo "   nano $PROJECT_DIR/.env"
    echo ""
    echo "3. Generate secure passwords:"
    echo "   openssl rand -base64 32  # For DB passwords"
    ...
    echo ""
    echo "5. Set up SSL certificate (if using domain):"
...
main() {
    log "Starting Ubuntu server setup for Sochi Travel deployment"
